...
export function SettingsPage({ data, onChange }: Props) {
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const members = data.members.map((member) => member.id === primary.id ? { ...member, planningEndAge: Number(form.get('planningEndAge')) } : member)
    void onChange({
      ...data,
    ...
    <div className="page-stack narrow-page">
      <section className="panel">
        ...
        <form className="settings-form" onSubmit={submit}>
          <fieldset><legend>時間範圍</legend><div className="form-grid two"><label>計算基準日<input name="calculationBaseDate" type="date" required defaultValue={data.calculationBaseDate} /></label><label>最早允許退休月份<input name="earliestRetirementMonth" type="month" required defaultValue={data.retirementPlan.earliestRetirementMonth} /></label><label>規劃終點年齡<input name="planningEndAge" type="number" min="70" max="120" required defaultValue={primary.planningEndAge} /></label></div></fieldset>
