...
export function RetirementSystemsPage({ data, estimates, onChange }: Props) {
  ...
  return <div className="page-stack">
    <section className="panel"><div className="panel-heading"><div><h2><Scale size={21} /> 勞保與勞退估算</h2><p>規則版本 {TAIWAN_LABOR_RULES_2026.version}，查核日 {TAIWAN_LABOR_RULES_2026.checkedAt}。結果僅供規劃，實際資格與金額以主管機關核定為準。</p></div></div><div className="source-links"><a href={TAIWAN_LABOR_RULES_2026.sources[0]} target="_blank" rel="noreferrer">勞保給付標準 <ExternalLink size={15} /></a><a href={TAIWAN_LABOR_RULES_2026.sources[2]} target="_blank" rel="noreferrer">勞退月退休金基礎 <ExternalLink size={15} /></a></div>{message && <div className="alert success" role="status">{message}</div>}</section>
