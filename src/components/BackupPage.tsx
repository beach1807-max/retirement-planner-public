...
export function BackupPage({ data, onRestore, onClear }: Props) {
  ...
  return (
    <div className="page-stack narrow-page">
      <section className="panel backup-hero"><div><span className="metric-icon large"><Download size={26} /></span><h2><CalculationHelp label="定期保留一份本機備份" topic="localData" /></h2><p>第一版不使用雲端同步。清除瀏覽器資料、移除網站資料或更換裝置，都可能失去目前規劃。</p></div><button className="button primary" onClick={() => downloadBackup(data)}><Download size={18} /> 匯出 JSON 備份</button></section>
