...
export function MarketDataPage({ data, service, onChange }: Props) {
  ...
  return <div className="page-stack">
    <section className="panel"><div className="panel-heading"><div><h2><RefreshCw size={21} /> 行情更新</h2><p>查看已連結標的狀態，並在需要時一次更新所有行情與匯率。失敗時會保留上次有效市值。</p></div><button className="button primary" disabled={updating || linkedAssets.length === 0} onClick={() => void refresh()}><RefreshCw size={18} />{updating ? '更新中…' : '更新所有行情'}</button></div><div className="source-links"><a href="https://openapi.twse.com.tw/" target="_blank" rel="noreferrer">臺灣證券交易所行情來源 <ExternalLink size={15} /></a><a href="https://cpx.cbc.gov.tw/Data/ExportToAPIInfo" target="_blank" rel="noreferrer">中央銀行匯率來源 <ExternalLink size={15} /></a></div>{activeReport && <div className={`alert ${activeReport.status === 'success' ? 'success' : activeReport.status === 'failed' ? 'error' : 'info'}`} role="status">{activeReport.status === 'success' ? '行情與匯率更新完成。' : activeReport.status === 'partial' ? '部分資料已更新，其餘保留上次有效值。' : '更新失敗，已保留上次有效值。'} {activeReport.errors.join(' ')}</div>}{staleQuotes.length > 0 && <div className="alert info">有 {staleQuotes.length} 筆行情超過 7 天，請勿視為最新價格。</div>}</section>
    <section className="panel"><div className="panel-heading"><div><h3><CalculationHelp label="已連結行情資產" topic="marketValue" /></h3><p>上市代碼、持有數量與帳戶請回「家庭資料」編輯資產，不需在這裡重複設定。</p></div></div><div className="data-list">{linkedAssets.map(({ instrument, asset, totalQuantity, holdingCount, quote }) => <article key={instrument.id}><div><strong>{asset?.name ?? '未知資產'} · {instrument.symbol}</strong><p>{holdingCount ? `${holdingCount} 個有效部位，共 ${totalQuantity}` : '尚無有效持有數量'} · {quote ? `行情日 ${quote.asOf}` : '等待首次更新'}</p></div><strong>{asset ? currency.format(Number(asset.currentValue.amount)) : '—'}</strong></article>)}{linkedAssets.length === 0 && <div className="empty-state">尚未連結行情資產。請到「家庭資料」新增或編輯股票／ETF。</div>}</div></section>
