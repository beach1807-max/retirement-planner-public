...
import { formatMoney, moneyToTwd } from '../application/money'
...
import { hasMassiveApiKey } from '../infrastructure/massive-api-key'
...
export function MarketDataPage({ data, service, onChange }: Props) {
  ...
  return <div className="page-stack">
    {data.instruments.some((instrument) => instrument.market === 'US') && !hasMassiveApiKey() && !hasUsMarketApiKey() && <div className="alert info">美股收盤價需要先到「預測設定」儲存 Massive 或 StashGamma API Key；兩者皆未設定時會保留手動市值。</div>}
    <section className="panel"><div className="panel-heading"><div><h2><RefreshCw size={21} /> 行情更新</h2><p>查看已連結標的狀態，並在需要時一次更新所有行情與匯率。失敗時會保留上次有效市值。</p></div><button className="button primary" disabled={updating} onClick={() => void refresh()}><RefreshCw size={18} />{updating ? '更新中…' : '更新所有行情'}</button></div><div className="source-links"><a href="https://openapi.twse.com.tw/" target="_blank" rel="noreferrer">上市行情：臺灣證券交易所 <ExternalLink size={15} /></a><a href="https://finmind.github.io/" target="_blank" rel="noreferrer">上櫃行情：FinMind <ExternalLink size={15} /></a><a href="https://massive.com/docs/rest/stocks/aggregates/previous-day-bar" target="_blank" rel="noreferrer">美股行情：Massive <ExternalLink size={15} /></a><a href="https://www.stashgamma.com/free-stock-data-api" target="_blank" rel="noreferrer">美股備援：StashGamma <ExternalLink size={15} /></a><a href="https://cpx.cbc.gov.tw/Data/ExportToAPIInfo" target="_blank" rel="noreferrer">匯率：中央銀行 <ExternalLink size={15} /></a></div>{activeReport && <div className={`alert ${activeReport.status === 'success' ? 'success' : activeReport.status === 'failed' ? 'error' : 'info'}`} role="status">{activeReport.status === 'success' ? '行情與匯率更新完成。' : activeReport.status === 'partial' ? '部分資料已更新，其餘保留上次有效值。' : '更新失敗，已保留上次有效值。'} {activeReport.errors.join(' ')}</div>}{staleQuotes.length > 0 && <div className="alert info">有 {staleQuotes.length} 筆行情超過 7 天，請勿視為最新價格。</div>}</section>
    <section className="panel"><div className="panel-heading"><div><h3><CalculationHelp label="已連結行情資產" topic="marketValue" /></h3><p>市場、代碼、持有數量與帳戶請回「家庭資料」編輯資產，不需在這裡重複設定。</p></div></div><div className="data-list">{linkedAssets.map(({ instrument, asset, totalQuantity, holdingCount, quote }) => <article key={instrument.id}><div><strong>{asset?.name ?? '未知資產'} · {instrument.market} · {instrument.symbol}</strong><p>{holdingCount ? `${holdingCount} 個有效部位，共 ${totalQuantity}` : '尚無有效持有數量'} · {quote ? `收盤日 ${quote.asOf}` : activeReport ? '尚未取得行情，請查看更新結果' : '等待首次更新'}</p></div><strong>{asset ? (moneyToTwd(data, asset.currentValue) !== null ? formatMoney({ amount: moneyToTwd(data, asset.currentValue)!.toFixed(2), currency: 'TWD' }) : '缺少匯率，無法換算 TWD') : '—'}{asset && asset.currentValue.currency !== 'TWD' && <small>原幣 {formatMoney(asset.currentValue)}</small>}</strong></article>)}{linkedAssets.length === 0 && <div className="empty-state">尚未連結行情資產。請到「家庭資料」新增或編輯股票／ETF。</div>}</div></section>
