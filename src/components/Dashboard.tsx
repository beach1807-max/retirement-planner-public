...
export function Dashboard({ data, systemEstimates, portfolio, projection: initialProjection, calculating, service }: Props) {
  ...
  const chartData = useMemo(() => projection?.horizons.filter((value) => showAll || value === years).map((years) => ({
    years: `${years} 年後`,
