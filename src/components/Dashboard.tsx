...
export function Dashboard({ data, systemEstimates, portfolio, projection: initialProjection, calculating, service, onContinueFullPlan }: Props) {
  ...
  return <div className="page-stack">
    ...
    <section className="panel">
      ...
      {projection ? <div className="projection-table-wrap"><table className="projection-table"><thead><tr><th>期間</th>{projection.scenarios.map((scenario) => <th key={scenario.id}>{scenario.id === 'optimistic' ? '比較樂觀' : scenario.label}<small>{scenario.id !== 'custom' ? '依各資產情境設定；未自設者 ' : '各資產穩健報酬再調整 '}{Number(scenario.returnAdjustment) >= 0 ? '+' : ''}{(Number(scenario.returnAdjustment) * 100).toFixed(1)} 個百分點</small></th>)}</tr></thead><tbody>{visibleYears.map((years) => <tr key={years}><th>{years} 年後<small>{projection.scenarios[0].milestones.find((item) => item.yearsFromNow === years)?.month}</small></th>{projection.scenarios.map((scenario) => { const item = scenario.milestones.find((item) => item.yearsFromNow === years)!; const nom = Number(item.totalAssetsNominal); const real = Number(item.totalAssetsReal); const discount = nom > 0 ? Math.round((1 - real / nom) * 100) : 0; return <td key={scenario.id} data-scenario={scenario.id === 'optimistic' ? '比較樂觀' : scenario.label}><strong>預估合計 {currency.format(nom)}</strong><small>投資與投入 {currency.format(Number(item.investmentAssetsNominal))}</small><small>勞退累積／請領時金額 {currency.format(Number(item.laborPensionAssetsNominal))}</small><small className="purchasing-power-tag">今天購買力 {currency.format(real)}{discount > 0 ? <span className="inflation-impact">（通膨折減 {discount}%）</span> : null}</small></td> })}</tr>)}</tbody></table></div> : !invalidAdjustment && !error && <p role="status">正在計算預測…</p>}
    </section>
