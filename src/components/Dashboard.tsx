...
export function Dashboard({ data, systemEstimates, portfolio, projection: initialProjection, calculating, service, financialOverview, onContinueFullPlan, onOpenData }: Props) {
  ...
  return <div className="page-stack">
    ...
    <section className="panel">
      <div className="panel-heading"><div><h3><Landmark size={20} /><CalculationHelp label="勞保年金" topic="laborInsurance" />與<CalculationHelp label="勞退專戶" topic="laborPension" /></h3><p>以下依所選 {years} 年後判斷一般年齡與年資條件；勞保另須離職退保。勞保固定月領；勞退依退休制度頁選擇一次領或月領。整體預測的勞退在請領後保留請領時點金額，並非剩餘專戶餘額；月領收入不再重複加總。</p></div><small>估算，實際資格與金額以主管機關核定為準</small></div>
      <div className="labor-system-grid">{systemEstimates.map((view) => { const eligibility = service.retirementEligibility(data, view.memberId, addMonths(data.calculationBaseDate, years * 12)); return <article key={view.memberId}><h4>{view.memberName}</h4><div><span>勞保年金（設定請領時預估）</span><strong>{view.laborInsuranceEnabled && eligibility && !eligibility.insurance.eligible ? '尚未符合請領條件' : view.laborInsuranceEnabled && view.estimate?.laborInsurance.status !== 'success' && view.status === 'provided' ? '請領設定或年資不符合規則' : view.laborInsuranceEnabled && view.estimate?.laborInsurance.monthlyBenefitRealTwd ? `${currency.format(Number(view.estimate.laborInsurance.monthlyBenefitRealTwd))}／月` : '資料未提供'}</strong><small>{view.estimate ? `${view.estimate.laborInsurance.claimMonth} 請領（${view.estimate.laborInsurance.statutoryClaimAge} 歲為法定年齡）` : '請至退休制度補齊'}</small></div><div><span>勞退請領時預估金額</span><small>{view.laborPensionEnabled && eligibility ? !eligibility.pension.eligible ? '尚未符合請領條件（一般須滿 ' + eligibility.pension.minimumAge + ' 歲）' : eligibility.pension.monthlyEligible ? '已達一般年齡與月領年資條件' : '年資未滿 15 年，僅適用一次領' : '資料未提供'}</small><strong>{view.laborPensionEnabled && view.estimate?.laborPension.projectedAccountBalanceTwd ? currency.format(Number(view.estimate.laborPension.projectedAccountBalanceTwd)) : '資料未提供'}</strong><small>{view.estimate ? `${view.estimate.laborPension.claimMonth} 預估請領 · ${view.estimate.laborPension.status === 'monthly' ? '月領，首期約 ' + currency.format(Number(view.estimate.laborPension.monthlyBenefitNominalTwd)) + '／月' : '一次領'}` : '請至退休制度補齊'}</small></div></article> })}</div>
    </section>

    <section className="panel"><div className="panel-heading"><div><h3>家庭財務概況</h3><p>以目前已提供的家庭收支、負債與帳戶資料彙整；未提供的資料不會被當成 0。</p></div><button className="button secondary" type="button" onClick={onOpenData}>查看／編輯家庭資料</button></div><div className="cashflow-grid">{([['每月收入', financialOverview.household.monthlyIncomeTwd], ['每月平均支出', financialOverview.household.monthlyExpenseTwd], ['每月負債還款', financialOverview.household.monthlyDebtPaymentTwd], ['每月投資', financialOverview.household.monthlyContributionTwd], ['每月可支配餘額', financialOverview.household.unallocatedTwd]] as const).map(([label, value]) => <article key={label}><span>{label}</span><strong>{value === undefined ? '尚未提供' : currency.format(Number(value))}</strong></article>)}</div><div className="cashflow-grid"><article><span>總資產</span><strong>{currency.format(Number(financialOverview.assets.totalTwd))}</strong></article><article><span>總負債</span><strong>{currency.format(Number(financialOverview.assets.liabilitiesTwd))}</strong></article><article><span>淨資產</span><strong>{currency.format(Number(financialOverview.assets.netWorthTwd))}</strong></article></div>{financialOverview.accounts.length > 0 && <><h4>帳戶摘要（共 {financialOverview.accountCount} 個帳戶）</h4><div className="data-list">{financialOverview.accounts.slice(0, 4).map((account) => <article key={account.id}><div><strong>{account.name}</strong><p>{account.institution ?? account.accountType} · {account.assetCount} 筆資產</p></div><strong>{account.totalTwd === undefined ? '尚未提供' : currency.format(Number(account.totalTwd))}</strong></article>)}</div>{financialOverview.accountCount > 4 && <p className="muted">另有 {financialOverview.accountCount - 4} 個帳戶。</p>}</>}</section>
