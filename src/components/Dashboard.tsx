...
import { PROJECTION_SCENARIO_LABELS } from '../content/terminology'
...
export function Dashboard({ data, systemEstimates, portfolio, projection: initialProjection, calculating, service, financialOverview, onContinueFullPlan, onOpenData }: Props) {
  ...
  const chartData = points.map((point) => { const balancedPoint = balanced ? milestoneFor(balanced, point) : undefined; return { point: point.target ? '目標退休時' : `${point.years} 年後`, ...Object.fromEntries(chartScenarios.map((scenario) => [PROJECTION_SCENARIO_LABELS[scenario.id], Number(milestoneFor(scenario, point)?.totalAssetsReal ?? 0)])), ...(scope === 'all' && projection?.liabilityResults.length ? { 剩餘負債: Number(balancedPoint?.remainingLiabilitiesReal ?? 0), 投資資產扣除負債: Number(balancedPoint?.investmentAssetsLessLiabilitiesReal ?? 0) } : {}) } })
  ...
  return <div className="page-stack">
    ...
    {scope === 'all' && projection?.liabilityResults.length ? <section className="panel debt-comparison">
      <div className="panel-heading"><div><h3><Landmark size={20} /><CalculationHelp label="負債清償與投資比較" topic="liabilityProjection" /></h3><p>不需要設定收入或逐月現金流；負債依還款條件自行下降，投資仍依你設定的每月投入成長，不會重複扣除貸款付款。</p></div></div>
      <div className="cashflow-grid"><article><span>目前總負債</span><strong>{currency.format(Number(financialOverview.assets.liabilitiesTwd))}</strong></article><article><span>全部負債預計清償</span><strong>{projection.allLiabilitiesPayoffMonth ?? '尚有負債待補條件'}</strong></article><article><span>{activePoint ? timePointLabel(activePoint, data, scope) : '所選時間'}剩餘負債</span><strong>{balancedMilestone ? currency.format(Number(balancedMilestone.remainingLiabilitiesNominal)) : '計算中'}</strong></article><article><span>穩健情境投資資產扣除負債</span><strong>{balancedMilestone ? currency.format(Number(balancedMilestone.investmentAssetsLessLiabilitiesNominal)) : '計算中'}</strong></article></div>
      <div className="data-list">{projection.liabilityResults.map((item) => <article key={item.id}><div><strong>{item.name}</strong><p>{item.status === 'calculated' ? `預計 ${item.payoffMonth} 清償` : item.status === 'manual' ? '待補利率、還款方式與期數' : '依目前設定無法清償'}</p></div><strong>{item.status === 'calculated' ? `利息 ${currency.format(Number(item.totalInterest))}` : '未納入未來比較'}</strong></article>)}</div>
      <p className="muted">此處比較的是公版投資組合與勞退，扣除已納入且可計算的負債；不等同包含房屋等全部財產的完整家庭淨資產。</p>
    </section> : null}
    ...
    <section className="panel">
      ...
      {projection ? <div className="projection-table-wrap"><table className="projection-table"><thead><tr><th>時間點</th>{projection.scenarios.map((scenario) => <th key={scenario.id} className={scenario.id === 'balanced' ? 'baseline-scenario' : undefined}>{PROJECTION_SCENARIO_LABELS[scenario.id]}<small>{rateSummary(scenario.investmentAnnualReturnRates)}</small>{scenario.laborPensionAnnualReturnRates.length > 0 && <small>勞退 {scenario.laborPensionAnnualReturnRates.map((rate) => `${(Number(rate) * 100).toFixed(1)}%`).join('～')}</small>}</th>)}</tr></thead><tbody>{visiblePoints.map((point) => <tr key={point.key}><th>{timePointLabel(point, data, scope)}</th>{projection.scenarios.map((scenario) => { const item = milestoneFor(scenario, point)!; const nominal = Number(item.totalAssetsNominal); const real = Number(item.totalAssetsReal); const discount = nominal > 0 ? Math.round((1 - real / nominal) * 100) : 0; return <td key={scenario.id} className={scenario.id === 'balanced' ? 'baseline-scenario' : undefined} data-scenario={PROJECTION_SCENARIO_LABELS[scenario.id]}><strong>名目資產 {currency.format(nominal)}</strong><small className="projection-real-value">今天購買力 {currency.format(real)}{discount > 0 ? <span className="inflation-impact">（通膨折減 -{discount}%）</span> : null}</small>{scope === 'all' && projection.liabilityResults.length > 0 && <><small>剩餘負債 {currency.format(Number(item.remainingLiabilitiesNominal))}</small><small>投資資產扣除負債 {currency.format(Number(item.investmentAssetsLessLiabilitiesNominal))}</small></>}<details className="projection-cell-details"><summary>查看組成與資格</summary><small>投資與投入 {currency.format(Number(item.investmentAssetsNominal))}</small><small>勞退帳面累積價值（請領後固定） {currency.format(Number(item.laborPensionAssetsNominal))}</small>{scope === 'all' && systemEstimates.filter((view) => view.laborPensionEnabled && view.status === 'provided').map((view) => { const eligibility = service.retirementEligibility(data, view.memberId, item.month)?.pension; return <small key={view.memberId}>{view.memberName} 勞退：{eligibility?.eligible ? '已達一般請領年齡' : `尚未符合請領條件（一般須滿 ${eligibility?.minimumAge} 歲）`}</small> })}</details></td> })}</tr>)}</tbody></table></div> : !invalidAdjustment && !error && <p role="status">正在計算預測…</p>}
    </section>

    <section className="content-grid">
      <article className="panel"><div className="panel-heading"><div><h3><CalculationHelp label="相當於今天多少錢？" topic="purchasingPower" /></h3><p>依上方資產範圍顯示固定期間與目標退休時間；整體範圍另顯示負債與穩健情境扣除負債後的今天購買力。</p></div></div><div className="chart-wrap" aria-label="各情境今天購買力與剩餘負債趨勢；下方預測表格提供完整數值"><ResponsiveContainer width="100%" height="100%"><LineChart data={chartData} margin={{ top: 8, right: 10, bottom: 4, left: 4 }}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="point" interval="preserveStartEnd" minTickGap={18} /><YAxis tickFormatter={compactCurrency} width={76} /><Tooltip formatter={(value) => currency.format(Number(value))} /><Legend />{chartScenarios.map((scenario) => <Line key={scenario.id} type="monotone" dataKey={PROJECTION_SCENARIO_LABELS[scenario.id]} stroke={scenarioColors[scenario.id]} strokeWidth={scenario.id === 'balanced' ? 4 : 2.5} dot={{ r: scenario.id === 'balanced' ? 4 : 3 }} isAnimationActive={false} />)}{scope === 'all' && projection?.liabilityResults.length ? <><Line type="monotone" dataKey="剩餘負債" stroke="#b4553d" strokeWidth={2.5} dot={{ r: 3 }} isAnimationActive={false} /><Line type="monotone" dataKey="投資資產扣除負債" stroke="#243b53" strokeWidth={3} dot={{ r: 3 }} isAnimationActive={false} /></> : null}</LineChart></ResponsiveContainer></div></article>
