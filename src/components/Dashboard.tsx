...
export function Dashboard({ data, viewModel, systemEstimates, portfolio, onScopeChange, projection, calculating }: Props) {
  ...
  return <div className="page-stack">
    ...
    <section className="panel">
      <div className="panel-heading"><div><h3><PieChart size={20} />目前投資組合配置</h3><p>以投資組合中選取的資產為範圍；ETF 與基金依實際投資內容分類。</p></div><small>{lastMarketUpdate ? `行情 ${new Date(lastMarketUpdate.completedAt).toLocaleDateString('zh-TW')}` : '使用目前登錄市值'}</small></div>
    ...
    <section className="panel">
      <div className="panel-heading"><div><h3><TrendingUp size={20} />固定期間資產預估</h3><p>名目金額是未來帳面金額；今天購買力已依年通膨率 {(Number(projection?.inflationRate ?? 0) * 100).toFixed(1)}% 換算。</p></div><small>{projection?.contractVersion ?? 'projection-contract-v0.2'}</small></div>
    ...
    <section className="content-grid">
      <article className="panel"><div className="panel-heading"><div><h3>今天購買力趨勢</h3><p>三條線使用相同本金與投入計畫，只調整年化報酬假設。</p></div></div><div className="chart-wrap"><ResponsiveContainer width="100%" height="100%"><LineChart data={chartData} margin={{ top: 8, right: 10, bottom: 4, left: 4 }}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="years" /><YAxis tickFormatter={compactCurrency} width={58} /><Tooltip formatter={(value) => currency.format(Number(value))} /><Legend />{projection?.scenarios.map((scenario) => <Line key={scenario.id} type="monotone" dataKey={scenario.label} stroke={scenarioColors[scenario.id]} strokeWidth={3} dot={{ r: 3 }} />)}</LineChart></ResponsiveContainer></div></article>
      <article className="panel"><div className="panel-heading"><div><h3>穩健情境組成</h3><p>投資資產與勞退專戶分開列示。</p></div></div><div className="milestone-grid projection-breakdown">{balanced?.milestones.map((item) => <article key={item.yearsFromNow}><span>{item.yearsFromNow} 年後</span><strong>{currency.format(Number(item.totalAssetsNominal))}</strong><small>投資 {currency.format(Number(item.investmentAssetsNominal))}</small><small>勞退 {currency.format(Number(item.laborPensionAssetsNominal))}</small></article>)}</div></article>
    </section>

    <section className="panel">
      <div className="panel-heading"><div><h3><Landmark size={20} />勞保年金與勞退專戶</h3><p>勞保是未來收入，不加入資產總額；勞退專戶列入上方資產預測。</p></div><small>估算，實際資格與金額以主管機關核定為準</small></div>
    ...
    <section className="panel secondary-summary"><div className="panel-heading"><div><h3><WalletCards size={20} />完整資產摘要</h3><p>保留完整個人／家庭資產管理，摘要不作為首頁主要判斷。</p></div></div><div className="cashflow-grid"><article><span>總資產</span><strong>{currency.format(Number(viewModel.totalAssetsTwd))}</strong></article><article><span>總負債</span><strong>{currency.format(Number(viewModel.totalLiabilitiesTwd))}</strong></article><article><span>淨資產</span><strong>{currency.format(Number(viewModel.netWorthTwd))}</strong></article></div></section>
