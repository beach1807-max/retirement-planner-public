    ...
    <section className="panel">
      ...
      {assetEditor && <form key={assetEditor} className="editor-form" onSubmit={saveAsset}>
        <h3>基本資料</h3><p className="muted">這是什麼、現在值多少、是誰的。</p><div className="form-grid three">
          ...
          <label>資產幣別<select value={assetCurrency} onChange={(event) => { setAssetCurrency(event.target.value); if (event.target.value === 'USD' && isMarketTrackableAssetType(assetType)) setMarketTracking(true) }}><option value="TWD">TWD 新台幣</option><option value="USD">USD 美元</option>{!['TWD', 'USD'].includes(assetCurrency) && <option value={assetCurrency}>{assetCurrency}</option>}</select></label>
          {assetCurrency === 'TWD' && isMarketTrackableAssetType(assetType) && <label>臺灣市場<select value={market} onChange={(event) => setMarket(event.target.value as SupportedMarket)}><option value="TWSE">上市（TWSE）</option><option value="TPEX">上櫃（TPEx）</option></select></label>}
          {assetCurrency === 'USD' && isMarketTrackableAssetType(assetType) && <fieldset><legend>美股收盤價追蹤</legend><label className="checkbox-row"><input type="checkbox" checked={marketTracking} onChange={(event) => setMarketTracking(event.target.checked)} />以美股日收盤價更新目前價值</label>{marketTracking && <div className="form-grid three"><label>美股代號<input value={symbol} onChange={(event) => setSymbol(event.target.value.toUpperCase())} pattern="[A-Za-z][A-Za-z0-9.\\-]{0,14}" placeholder="例如 VTI" required /></label><label>持有數量<input name="quantity" type="number" min="0" step="any" defaultValue={editedAsset && data.holdings.find((item) => item.assetId === editedAsset.id)?.quantity} required /></label><label>所屬帳戶（選填）<select name="accountId" defaultValue={editedAsset?.accountId}><option value="">行情追蹤帳戶</option>{data.accounts.map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select></label></div>}<p className="muted">只更新已完成交易日的收盤價；若行情服務尚未啟用，會保留手動市值。</p></fieldset>}
