  ...
  function saveAsset(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      next = upsertAssetMarketLink(withAsset, {
        ...
        symbol,
        market: assetCurrency === 'USD' ? 'US' : market === 'TPEX' ? 'TPEX' : 'TWSE',
        quantity: String(form.get('quantity') ?? ''),
        accountId: String(form.get('accountId') ?? '') || undefined,
      })
    } catch (caught) {
      setError(caught instanceof Error && caught.message === 'MISSING_HOLDING_QUANTITY' ? '請輸入持有數量。' : assetCurrency === 'USD' ? '請輸入有效的美股代號。' : '請輸入有效的臺灣市場代碼。')
      return
    }
    void onChange(next)
  ...
  return <div className="page-stack"><section className="panel"><h2>家庭完整資產摘要</h2><p className="muted">這是完整財產紀錄，與投資組合的預測範圍不同；共同資產只計一次。</p><div className="cashflow-grid"><article><CalculationHelp label="完整資產總額" topic="totalAssets" /><strong>{currency.format(Number(summary.totalAssetsTwd))}</strong></article><article><CalculationHelp label="總負債" topic="totalLiabilities" /><strong>{currency.format(Number(summary.totalLiabilitiesTwd))}</strong></article><article><CalculationHelp label="淨資產" topic="netWorth" /><strong>{currency.format(Number(summary.netWorthTwd))}</strong></article></div></section>
    ...
    <section className="panel">
      ...
      {assetEditor && <form key={assetEditor} className="editor-form" onSubmit={saveAsset}>
        <h3>基本資料</h3><p className="muted">這是什麼、現在值多少、是誰的。</p><div className="form-grid three">
        ...
        </div><details className="advanced-settings asset-prediction-settings" open={predictionSettingsOpen} onToggle={(event) => setPredictionSettingsOpen(event.currentTarget.open)}><summary>投資預測與行情設定（選填）</summary><h3>投資預測相關設定</h3><p className="muted">分類與報酬不代表已加入預測。儲存後請到「投資組合」選取。</p><div className="context-help-row" aria-label="投資預測欄位說明"><CalculationHelp label="所有權與共同持分" topic="ownership" /><CalculationHelp label="投資配置分類" topic="assetAllocation" /><CalculationHelp label="報酬情境" topic="forecastScenarios" /><CalculationHelp label="外幣匯率" topic="exchangeRate" /></div><div className="form-grid two">
          ...
          <label>資產幣別<select value={assetCurrency} onChange={(event) => { const currency = event.target.value; setAssetCurrency(currency); if (currency === 'USD') setMarket('US'); else if (currency === 'TWD') setMarket('TWSE') }}><option value="TWD">TWD 新台幣</option><option value="USD">USD 美元</option>{!['TWD', 'USD'].includes(assetCurrency) && <option value={assetCurrency}>{assetCurrency}</option>}</select></label>
          {assetCurrency === 'TWD' && isMarketTrackableAssetType(assetType) && <label>掛牌市場<select value={market} onChange={(event) => setMarket(event.target.value as SupportedMarket)}><option value="TWSE">上市（TWSE）</option><option value="TPEX">上櫃（TPEx）</option></select></label>}
          {assetCurrency === 'USD' && isMarketTrackableAssetType(assetType) && <fieldset><legend>美股收盤價追蹤</legend><label>掛牌市場<select value={market} onChange={() => setMarket('US')}><option value="US">美國（US）</option></select></label><label className="checkbox-row"><input type="checkbox" checked={marketTracking} onChange={(event) => setMarketTracking(event.target.checked)} />以美股日收盤價更新目前價值</label>{marketTracking && <div className="form-grid three"><label>美股代號<input value={symbol} onChange={(event) => setSymbol(event.target.value.toUpperCase())} pattern="[A-Za-z][A-Za-z0-9.-]{0,14}" placeholder="例如 VTI" required /></label><label>持有數量<input name="quantity" type="number" min="0" step="any" defaultValue={editedAsset && data.holdings.find((item) => item.assetId === editedAsset.id)?.quantity} required /></label><label>所屬帳戶（選填）<select name="accountId" defaultValue={editedAsset?.accountId}><option value="">行情追蹤帳戶</option>{data.accounts.map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select></label></div>}<p className="muted">掛牌市場與投資區域分開設定。只更新已完成交易日的收盤價；若行情服務尚未啟用，會保留手動市值。</p></fieldset>}
          ...
          <label>{UI_TERMS.retirementNeedsUsageScope}<select name="retirementUsageScope" defaultValue={editedAsset?.retirementUsageScope}><option value="personal">個人退休使用</option><option value="household">家庭退休可用</option><option value="excluded">不納入退休</option></select></label>
