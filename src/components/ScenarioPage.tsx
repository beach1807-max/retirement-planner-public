...
export function ScenarioPage({ data, service, onChange }: Props) {
  ...
  return <div className="page-stack"><section className="panel"><div className="panel-heading"><div><h2><FlaskConical size={21} /> <CalculationHelp label="投入方案比較" topic="contribution" /></h2><p>比較不同投入計畫對未來資產的影響，不判斷何時應該退休。</p></div></div></section>
    ...
    <section className="panel"><div className="panel-heading"><div><h3><CalculationHelp label="未來資產比較" topic="purchasingPower" /></h3><p>全部以穩健情境及今天購買力呈現，可調整投入計畫來比較。</p></div></div>{!results ? <p role="status">正在計算情境…</p> : <div className="scenario-table-wrap"><table className="scenario-table"><thead><tr><th>方案</th><th>10 年後</th><th>20 年後</th><th>30 年後</th><th>35 年後</th><th>操作</th></tr></thead><tbody>{results.map((result) => <tr key={result.scenarioId}><th>{result.name}{result.warnings.map((warning) => <small key={warning}>{warning}</small>)}</th>{([10, 20, 30, 35] as const).map((years) => <td key={years} data-years={`${years} 年後`}>{result.futureAssetsReal[years] ? currency.format(Number(result.futureAssetsReal[years])) : '—'}</td>)}<td>{result.scenarioId !== 'baseline' && <button className="icon-button danger" aria-label={`刪除方案 ${result.name}`} onClick={() => void remove(result.scenarioId)}><Trash2 size={18} /></button>}</td></tr>)}</tbody></table></div>}</section>
  </div>
}
