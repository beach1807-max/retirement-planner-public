...
export function ScenarioPage({ data, service, onChange }: Props) {
  ...
  function create(event: FormEvent<HTMLFormElement>) {
    ...
    if (!retirement && (!additional || Number(additional) === 0) && !inflation && !voluntary && !laborClaimAge && !pensionClaimAge && !pensionClaimMode && !contributionAmount && !assetBalanced && rebalance.length === 0) { setError('至少需要一項情境覆寫。'); return }
    const overrides: PlannerScenario['overrides'] = {
      ...
      contributionOverrides: contributionId && contributionAmount ? [{ contributionId, amountTwd: contributionAmount }] : undefined,
  ...
  return <div className="page-stack">
    ...
    <section className="panel"><h3>建立情境</h3><form className="settings-form" onSubmit={create}><div className="form-grid three"><label>方案名稱<input name="name" required placeholder="例如：每月多投入一萬元" /></label><label>主要規劃人計畫退休月份<input name="plannedRetirementMonth" type="month" /></label><label>每月額外投入（TWD）<input name="additionalContribution" type="number" min="0" defaultValue="0" /></label><label>通膨率（%）<input name="annualInflationRate" type="number" min="-99" max="100" step="0.1" /></label><label>勞退自提率（%）<input name="voluntaryRate" type="number" min="0" max="6" step="0.5" /></label></div><details><summary>完整覆寫選項</summary><div className="form-grid three"><label>既有投入<select name="contributionId"><option value="">不調整</option>{data.contributions.map((item) => <option key={item.id} value={item.id}>{item.id.slice(0, 8)}</option>)}</select></label><label>覆寫既有投入金額<select name="contributionAmount"><option value="">不調整</option>{['0', '5000', '10000', '20000'].map((value) => <option key={value} value={value}>{value}</option>)}</select></label><label>資產報酬覆寫<select name="assetId"><option value="">不調整</option>{data.assets.map((asset) => <option key={asset.id} value={asset.id}>{asset.name}</option>)}</select></label><label>保守／穩健／樂觀報酬（%）<input name="assetConservative" type="number" step="0.1" placeholder="保守" /><input name="assetBalanced" type="number" step="0.1" placeholder="穩健" /><input name="assetOptimistic" type="number" step="0.1" placeholder="樂觀" /></label><label>勞保請領年齡<input name="laborClaimAge" type="number" min="60" max="70" /></label><label>勞退請領年齡<input name="pensionClaimAge" type="number" min="60" max="70" /></label><label>勞退請領方式<select name="pensionClaimMode"><option value="">不調整</option><option value="lumpSum">一次領</option><option value="monthly">月領</option></select></label></div><fieldset><legend>立即再平衡目標（合計必須為 100%）</legend><div className="form-grid five">{([['stock', '股票'], ['bond', '債券'], ['moneyMarket', '貨幣市場'], ['cash', '現金'], ['other', '其他']] as const).map(([key, label]) => <label key={key}>{label}%<input name={`target-${key}`} type="number" min="0" max="100" step="0.1" /></label>)}</div></fieldset></details>{error && <div className="alert error" role="alert">{error}</div>}<div className="form-actions"><button className="button primary" type="submit"><Plus size={18} />暫存試算</button></div></form></section>
