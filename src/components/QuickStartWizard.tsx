  ...
  return <main className="onboarding-shell">
    ...
    <section className="onboarding-card">
      ...
      <form className="form-stack" onSubmit={submit}>
        ...
        {step === 2 && <><div className="section-heading"><h2>目前可投入多少？</h2><p>只填可用於投資的資產；不必填收入、支出、房貸、勞保或勞退。</p></div><label>目前可投資資產總額（TWD）<CurrencyInput ariaLabel="目前可投資資產總額（TWD）" value={assets} required onChange={setAssets} /></label><label>每月預計投入金額（TWD）<CurrencyInput ariaLabel="每月預計投入金額（TWD）" value={monthly} required onChange={setMonthly} /></label></>}
