  ...
  return <main className="onboarding-shell">
    ...
    <section className="onboarding-card">
      ...
      <form className="form-stack" onSubmit={submit}>
        ...
        {step === 2 && <><div className="section-heading"><h2>目前可投入多少？</h2><p>只填可用於投資的資產；不必填收入、支出、房貸、勞保或勞退。</p></div><label><CalculationHelp label="目前可投資資產總額（TWD）" topic="investmentAssets" /><CurrencyInput ariaLabel="目前可投資資產總額（TWD）" value={assets} required onChange={setAssets} /></label><label><CalculationHelp label="每月預計投入金額（TWD）" topic="contribution" /><CurrencyInput ariaLabel="每月預計投入金額（TWD）" value={monthly} required onChange={setMonthly} /></label></>}
        {step === 3 && <><div className="section-heading"><h2><CalculationHelp label="目前資產怎麼分配？" topic="assetAllocation" /></h2><p>可填 0%，但五項合計必須剛好為 100%。</p></div><div className="form-grid two">{fields.map((field) => <label key={field.key}>{field.label}（%）<input aria-label={`${field.label}（%）`} type="number" min="0" max="100" step="1" inputMode="decimal" value={allocations[field.key]} onChange={(event) => setAllocations((value) => ({ ...value, [field.key]: event.target.value }))} /></label>)}</div><div className={allocationTotal === 100 ? 'alert success' : 'alert error'} role={allocationTotal === 100 ? 'status' : 'alert'}>目前合計：{allocationTotal}% {allocationTotal < 100 ? `｜還差：${100 - allocationTotal}%` : allocationTotal > 100 ? `｜超過：${allocationTotal - 100}%` : '｜配置完成'}</div></>}
