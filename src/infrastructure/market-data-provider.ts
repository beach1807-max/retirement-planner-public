...
export class OfficialTaiwanMarketDataProvider implements MarketDataProvider {
  readonly id = 'tw-us-eod-market-data-v0.10'
  ...
  async fetchLatest(instruments: MarketInstrumentRequest[]): Promise<MarketDataBatch> {
    ...
    const response = await fetch(`${this.endpoint}?instruments=${encodeURIComponent(supported.map((item) => `${item.market}:${item.providerSymbol ?? item.symbol}`).join(','))}`)
    if (!response.ok) throw new Error(`MARKET_PROVIDER_HTTP_${response.status}`)
    const payload = await response.json() as ApiResponse
    const quotes = payload.quotes.flatMap((quote) => {
      return supported.filter((item) => item.symbol === quote.symbol && item.market === ((quote as { market?: string }).market ?? 'TWSE')).map((instrument) => ({ ...quote, instrumentId: instrument.id }))
    })
