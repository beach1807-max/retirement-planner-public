...
it('VT 使用瀏覽器本機免費金鑰取得美股最新日收盤價', async () => {
  ...
  const result = await new StashGammaUsMarketDataProvider().fetchLatest([{ id: 'vt', symbol: 'VT', market: 'US', currency: 'USD' }])
  expect(result.quotes).toEqual([{ instrumentId: 'vt', symbol: 'VT', price: '230.5', currency: 'USD', asOf: '2026-09-10', sourceId: 'stashgamma-eod' }])
  expect(String(fetchMock.mock.calls[0][0])).toContain('/eod/VT')
  expect(fetchMock.mock.calls[0][1].headers).toEqual({ 'X-Api-Key': 'sg_live_test' })
})
...
it('未設定美股金鑰時保留手動市值並回報設定提示', async () => {
  ...
  const result = await new StashGammaUsMarketDataProvider().fetchLatest([{ id: 'vti', symbol: 'VTI', market: 'US', currency: 'USD' }])
  expect(result.quotes).toEqual([])
  expect(result.errors.map((error) => error.message)).toContain('VTI 未設定 StashGamma API Key，已保留上次有效行情或手動市值。')
