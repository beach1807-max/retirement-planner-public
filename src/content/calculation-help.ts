...
export const calculationHelp = {
  ...
  forecastScenarios: { title: '保守／穩健／樂觀情境', summary: '三種情境使用相同本金與投入計畫，只調整每筆投資及勞退的年化報酬率。', formula: '保守 = 原報酬率 − 2%；穩健 = 原報酬率；樂觀 = 原報酬率 + 2%', note: '此處的 2% 是 2 個百分點，情境不代表機率或最差／最好結果。' },
  ...
  annualReturn: { title: '有效年化報酬率', summary: '表示一年複利後的總報酬假設，系統會換算成等效月報酬率。', formula: '月報酬率 = (1 + 年化報酬率)^(1/12) − 1', note: '固定報酬假設未反映市場逐年波動、稅費或報酬順序風險。' },
  inflation: { title: '年化通膨率', summary: '用來把未來名目金額折算為今天購買力，也用於舊版退休支出的逐月調整。通膨率愈高，相同未來金額的今天購買力愈低。' },
  ...
  insuredYears: { title: '勞保年資', summary: '用於勞保老年年金兩式計算。年資未滿 15 年時，系統不估算老年年金，需另行確認一次給付資格。' },
