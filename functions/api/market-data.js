...
export async function onRequestGet(context) {
  ...
  for (const market of ['TWSE', 'TPEX']) { const symbols = instruments.filter((item) => item.market === market).map((item) => item.symbol); if (symbols.length) try { quotes.push(...await taiwan(market, symbols)) } catch (error) { errors.push({ market, code: error instanceof Error ? error.message : `${market}_UNKNOWN`, message: `${market === 'TWSE' ? '臺灣證券交易所' : '證券櫃檯買賣中心'}收盤價暫時無法取得。` }) } }
